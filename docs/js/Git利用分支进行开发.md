![](http://7xinjg.com1.z0.glb.clouddn.com/git-coll-brhero.svg)
...
在阅读文本时，请记住 Git 的分支与 SVN 分支的不同。SVN 的分支是用来处理会进行的大规模开发才使用的，一般的项目都会进行多分支处理（branch，sit，master分别对应本地，sit，和线上或者灰度环境），而 Git 的分支确是我们日常工作流不可或缺的一部分。
...
git branch <branch>
```
该命令表示创建一个叫做 <branch> 的新分支，但 <b>不会</b> 检出到新分支上。
```
git branch -d <branch>
```
...
![](http://7xinjg.com1.z0.glb.clouddn.com/git-coll-br01.svg)
...
这对于理解Git的log和提交历史有着至关重要的作用。在 SVN 中合并是基于文件的，而 Git 让合并基于提交这一更加抽象的层面上。我们可以在项目历史中确切的看到：合并是作为两个独立的提交历史的连接的。
...
![](http://7xinjg.com1.z0.glb.clouddn.com/git-coll-br02.svg)

然后我们使用下面的命令创建分支：
```
git branch test-exprience
```
分支历史依然没有任何更改，我们的到的仅仅是一个指向当前提交的新指针：

![](http://7xinjg.com1.z0.glb.clouddn.com/git-coll-br03.svg)

注意到这一操作仅仅 创建了 **新分支**。如果想要在新分支上添加提交，我们需要使用 ``git checkout`` 命令选择它，并使用标准的 ``git add`` 和 ``git commit`` 命令，更多信息请参阅下一章节 ``git checkout``。

#### 删除分支

当我们结束在一个分支上的工作并将其合并到主干代码上时，我们就可以自由的删除分支而不丢失任何历史记录：
```
git branch -d test-exprience
```
然而，如果该分支还没有被合并，上面的命令会输出如下的信息：
```
error: The branch 'test-exprience' is not fully merged.
...
```
检出制定的分支，这一分支必须已经被 ``git branch`` 命令创建。<another-branch> 会成为当前分支，且会将工作目录更新至匹配状态。
```
...
#### 详述
Git在切换分支前会检查当前分支是否clean，如有没有未提交的内容（add 但未commit的内容），所以事先可以使用``git status``检查当前分支是否有未提交的内容。
检查clean后，就可以使用``git checkout <branch>``来进行分支间的切换了。

有一点需要注意的是，如果新增文件在当前分支没有被add，执行``git checkout``并不会报错，因为文件未添加到git版本管理中。文件会在新分支中也会继续存在，容易出现的问题是，这时候如果把文件提交到当前分支对应的版本库中，就会造成版本库文件不一致的情况，所以，开发分支合并到master或者test分支应尽量使用merge的方式，而不是自由的添加文件。

关于``git merge``的使用和详述，我会在以后单独阐述。